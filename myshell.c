...
int child_is_executing;

/* Intercepts SIGINT signal and sends it to foreground processes */

void sigint_handler(int signo) {
    if (!child_is_executing) {
        exit(EXIT_FAILURE);
    }
}

/* SIGINT handler for the children of the shell process */

void handler_children(int signo) {
    exit(EXIT_FAILURE);
}
...
int main(void) {
    ...
    child_is_executing = 0;                                 // Sets 'child_is_executing' flag to false
