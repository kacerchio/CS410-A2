...
int main(int argc, const char * argv[]) {
    ...
    //arbitrary number of lines for allocation. Memory reallocation is implemented, however, program cannot sort list of paths of size greater than lines_allocated.
    ...
    if (paths == NULL) {                                      //error checking for memory allocation.
    ...
    //opening file "paths.txt" containing paths for reading, which tsp_p and tsp_t will write to. For now, full directory path is used.
    
    FILE *fp = fopen("paths.txt", "r");
    
    
    if (fp == NULL) {                                        //Error checking for opening file.
    ...
    for (i = 0; 1; i++) {
        ...
        // Checking if i has exceeded maximum lines allocated for the paths. Checking if reallocation of memory is required.
        if (i >= lines_allocated) {
            ...
            allocfactor = allocfactor + 1;                   //Allocfactor keeps track of any reallocation so size of totaldist array is accurate.
        ...
        if (fgets(paths[i], max_line_len-1, fp) == NULL) {   // Reading in a line from the file, making sure it is not NULL.
            break;
        }
        
        // When reading in a line, the new line or carriage return character at the end of the line is also read in, which means there will be a double line space. This removes the new line or carriage return character from the end of the char string.
